...
CREATE PROCEDURE `usp_get_employees_salary_above`(`number` DECIMAL(15,4) )
BEGIN
	SELECT `first_name`,`last_name` FROM `employees`
	WHERE `salary` >= `number`
	ORDER BY `first_name`,`last_name`,`employee_id`;
...
CREATE FUNCTION  `ufn_get_salary_level`(`salary` DECIMAL(11,2))
RETURNS VARCHAR(10)
DETERMINISTIC
BEGIN
	RETURN (
...
BEGIN
	SELECT `first_name`,`last_name` FROM `employees`
	WHERE `param` = (SELECT ufn_get_salary_level(`salary`)) 
