...
-- 9. Employee Manager
SELECT `employee_id`,`first_name`,`manager_id`, (SELECT `first_name` FROM `employees` AS e2 WHERE e.`manager_id`=e2.`employee_id`) AS `manager_name` FROM `employees` AS e
...
SELECT 
    ...
    (SELECT 
        ...
        FROM
            `employees` AS e2
        WHERE
            e.`manager_id` = e2.`employee_id`) AS `manager_name`,
    d.`name` AS `department_name`
FROM
    `employees` AS e
        JOIN
