...
SELECT `first_name`,`last_name`,`salary`*2,LENGTH(`first_name`) AS `coach_level` FROM `players` 
WHERE `age`>=45;
...
SELECT 
    c.`name`,COUNT(p.`id`) AS `total_count_of_players`, SUM(p.`salary`) AS total_sum_of_salaries
FROM
    `players` AS p
       RIGHT JOIN
    `teams` AS t ON p.`team_id` = t.`id`
    ...
    `towns` AS tw ON s.`town_id` = tw.`id`
		RIGHT JOIN
	`countries` AS c ON tw.`country_id`=c.`id`
    GROUP BY c.`id`
...
BEGIN
	SELECT CONCAT_WS(' ',`first_name`,`last_name`) AS `full_name`,p.`age`,p.`salary`,sd.`dribbling`,sd.`speed`,t.`name` AS `team_name` FROM `players` AS p
	JOIN `skills_data` AS sd
	ON p.`skills_data_id`=sd.`id`
	JOIN `teams` AS t
	ON p.`team_id`=t.`id`
	WHERE sd.`dribbling`>`min_dribble_points` AND t.`name`=`team_name` 
	AND (SELECT AVG(sd.`speed`) FROM `players` AS p
	JOIN `skills_data` AS sd
	ON p.`skills_data_id`=sd.`id`)<sd.`speed`
