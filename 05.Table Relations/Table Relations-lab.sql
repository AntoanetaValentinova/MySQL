...
-- 2. Trip Organization
SELECT `driver_id`,`vehicle_type`,CONCAT(`first_name`,' ',`last_name`) AS `driver_name` FROM `campers`
JOIN `vehicles`
ON `campers`.`id`=`vehicles`.`driver_id`;

-- 3. SoftUni Hiking
SELECT `starting_point` AS `route_starting_point`,`end_point` AS `route_ending_point`,`campers`.`id` AS `leader_id`, CONCAT(`first_name`,' ',`last_name`) AS `leader_name` FROM `campers`
JOIN `routes`
